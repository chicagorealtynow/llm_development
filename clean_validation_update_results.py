...
def clean_text(text: str) -> str:
    ...
    - remove hyphen line breaks (word-\nwrap)
    ...
    # Remove hyphen + line break (e.g., "interoperabil-\nity" -> "interoperability")
    text = re.sub(r"-\s*\n", "", text)

    # Normalize Windows / Mac newlines just in case
    text = text.replace("\r\n", "\n").replace("\r", "\n")

    # Collapse 3+ newlines to 2 (keeps paragraph breaks but removes crazy spacing)
    text = re.sub(r"\n{3,}", "\n\n", text)
...
def clean_docs(docs, min_length: int = 10):
    """
    Apply cleaning to all docs and optionally drop very short chunks.
    Returns a new list of cleaned Document objects.
    """
    cleaned_docs = []
    for d in docs:
        ...
        # Optionally drop ultra-short chunks that are likely noise
    ...
    print(f"[CLEAN] Kept {len(cleaned_docs)} of {len(docs)} docs after cleaning/filtering")
    return cleaned_docs


# ---------- Main validator ----------

def validate_docs(docs, label: str = "CLEANLINESS VALIDATION"):
    """
...
if __name__ == "__main__":
    # 1) Load raw docs
    raw_docs = load_pdfs(path=".")

    # 2) Validate BEFORE cleaning
    validate_docs(raw_docs, label="CLEANLINESS VALIDATION (BEFORE CLEANING)")

    # 3) Deep copy and clean docs
    cleaned_docs = clean_docs(deepcopy(raw_docs), min_length=10)

    # 4) Validate AFTER cleaning
    validate_docs(cleaned_docs, label="CLEANLINESS VALIDATION (AFTER CLEANING)")

    # Optional: inspect a specific chunk if you want
    # print(cleaned_docs[8].page_content)
    # print(cleaned_docs[16].page_content)
